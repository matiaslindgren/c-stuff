...
### Usage
```
./build/O2-none/tools/dataset cifar_to_png dataset_path output_path [-v]
./build/O2-none/tools/dataset spambase dataset_path output_path [-v]
./build/O2-none/tools/dataset rcv1 dataset_path output_path [-v]
```
...
### Usage
```
./build/O2-none/tools/png info png_path
./build/O2-none/tools/png dump_raw png_path block_type [block_types...]
./build/O2-none/tools/png segment png_src_path png_dst_path [--threshold-percent=N] [-v]
```
...
```
./build/O2-none/tools/png info ./docs/img/tokyo.png | jq .
```
**`stdout`**:
```
{
...
#### Threshold 10%
```
./build/O2-none/tools/png segment \
...
```
./build/O2-none/tools/png dump_raw ./test-data/png/ff0000-1x1-rgb-fixed.png IHDR IDAT | xxd -b
```
**`stdout`**:
```
...
### Usage
```
./build/O2-none/tools/sort { numeric | ascii } path
```
...
```
./build/O2-none/tools/sort numeric ./test-data-sizes.txt
```
**`stdout`**:
```
69
...
```
./build/O2-none/tools/sort ascii ./test-data-sizes.txt
```
**`stdout`**:
```
11223
...
```
./build/O2-none/tools/sort numeric --reverse ./test-data-sizes.txt
```
**`stdout`**:
```
24733
...
[source](/tools/svm.c)
```
./build/O2-none/tools/svm experiment dataset_dir [-v]
```
...
```bash
./build/O2-none/tools/svm spambase out -v &| jq .
```
output
```json
{
...
### Usage
```
./build/O2-none/tools/txt concat path [paths...]
./build/O2-none/tools/txt count pattern path
./build/O2-none/tools/txt slicelines begin count path
./build/O2-none/tools/txt replace pattern replacement path
./build/O2-none/tools/txt linefreq path
```
...
#### Concatenate
```
./build/O2-none/tools/txt concat ./test-data/txt/wikipedia/water_{ja,is,hi,zh}.txt
```
**`stdout`**:
```
...
```
./build/O2-none/tools/txt count 'struct' tools/txt.c
./build/O2-none/tools/txt count '##' README.md
./build/O2-none/tools/txt count 'ある' README.md
./build/O2-none/tools/txt count 'ið' README.md
```
**`stdout`**:
```
36
...
#### Slice lines
```
./build/O2-none/tools/txt slicelines 315 10 ./tools/txt.c
```
**`stdout`**:
```
...
#### Replace pattern
```
./build/O2-none/tools/txt replace '水' water ./test-data/txt/wikipedia/water_ja.txt
```
**`stdout`**:
```
...
```
clang-20 -std=c23 -E -I. ./tools/txt.c \
  | ./build/O2-none/tools/txt replace '  ' '' /dev/stdin \
  | ./build/O2-none/tools/txt replace $'\n ' $'\n' /dev/stdin \
  | ./build/O2-none/tools/txt linefreq /dev/stdin \
  | ./build/O2-none/tools/sort numeric --reverse /dev/stdin \
  | ./build/O2-none/tools/txt slicelines 0 25 /dev/stdin
```
**`stdout`**:
```
278 }
...
9 return dst;
8 done:
7 __attribute__ ((__nonnull__ (1)));
7 for (size_t i = 0; i < n; ++i) {
6 # 1 "/usr/include/assert.h" 1 3 4
...
```sh
./build/O2-none/tools/png dump_raw ./docs/img/tokyo.png tEXt \
  | ./build/O2-none/tools/txt replace date: $'\n'date= /dev/stdin \
  | ./build/O2-none/tools/txt replace exif: $'\n'exif= /dev/stdin \
  | ./build/O2-none/tools/txt replace 0x00 ': ' /dev/stdin \
  && echo
```
**`stdout`**:
```
...
Check if a JSON document is valid or not.

### Usage
```
./build/O2-none/tools/json --check json_path
```
