...
bool test_tokenize_empty(const bool verbose) {
  ...
  struct sl_data token1 = sl_tokenizer_next_token(&empty1, &empty2, 0);
  assert(!token1.size);
  assert(!token1.data);

  struct sl_data token2 = sl_tokenizer_next_token(&data, &empty1, 0);
  assert(token2.size == 3);
  assert(token2.data == x);

  struct sl_data token3 = sl_tokenizer_next_token(&empty1, &data, 0);
  assert(!token3.size);
  assert(!token3.data);
...
bool test_tokenize_one(const bool verbose) {
  ...
  struct sl_data token1 = sl_tokenizer_next_token(&data1, &data2, 0);
  assert(token1.size == 1);
  assert(token1.data == x1);

  struct sl_data token2 = sl_tokenizer_next_token(&data1, &data2, 1);
  assert(!token2.size);
  assert(!token2.data);

  struct sl_data token3 = sl_tokenizer_next_token(&data1, &data2, 2);
...
bool test_tokenize_delimiters(const bool verbose) {
  ...
  for (size_t begin = 0; begin < 3; ++begin) {
    struct sl_data token = sl_tokenizer_next_token(&data1, &data2, begin);
    assert(!token.size);
    assert(!token.data);
  }
...
bool test_tokenize_iter(const bool verbose) {
  ...
  struct sl_data data = sl_data_view(SL_ARRAY_LEN(data_str), data_str);
  struct sl_data delimiter =
      sl_data_view(SL_ARRAY_LEN(delimiter_str), delimiter_str);
