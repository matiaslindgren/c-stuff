...
bool sl_record_validate_metadata(
  ...
  if (r->n_dims <= 0) {
    SL_ERROR(ctx, "n_dims must be positive");
    return false;
  }
  ...
  if (!is_sparse && !is_dense) {
    SL_ERROR(ctx, "unknown data layout '%s'", r->layout);
    return false;
  }
  if (is_dense && !r->size) {
    SL_ERROR(ctx, "dense data layout must have a size");
    return false;
  }

  if (sl_record_item_size(r) == 0) {
    SL_ERROR(ctx, "unknown data type '%s'", r->type);
    return false;
  }
  ...
  for (int d = 0; d < r->n_dims; ++d) {
    if (r->dim_size[d] == 0) {
      SL_ERROR(ctx, "unexpected non-zero size for dimension %d", d);
      return false;
    }
  }
#endif
...
bool sl_record_read_metadata(
  ...
  struct sl_file file = {0};
  {
    ...
    if (!sl_file_open(ctx, &file, full_path, "r")) {
      goto done;
    }
  }
  if (strlen(path) >= sizeof(record->path) || strlen(name) >= sizeof(record->name)) {
    SL_ERROR(ctx, "path or name too long");
    goto done;
  }
  ...
  if (EOF
      ...
      || record->n_dims == 0) {
    SL_ERROR(ctx, "failed reading %s", file.path);
    goto done;
  }
  ...
  if (!sl_record_validate_metadata(ctx, record)) {
    goto done;
  }
...
bool sl_record_write_metadata(
...

  {
    ...
    if (!sl_file_format_path(
      ...
      SL_ERROR(ctx, "cannot format metadata path '%s/%s'", record->path, record->name);
      goto done;
    }
    if (!sl_file_open(ctx, &file, full_path, "w")) {
      SL_ERROR(ctx, "cannot create record metadata file '%s'", file.path);
      goto done;
    }
  }
  ...
  if (0 > fprintf(
    ...
    SL_ERROR(ctx, "failed writing to record metadata '%s'", file.path);
    goto done;
  }

  for (int d = 0; d < record->n_dims; ++d) {
    if (0 > fprintf(file.file, "dim%d: %zu\n", d, record->dim_size[d])) {
      SL_ERROR(ctx, "failed writing dimension %d to '%s'", d, file.path);
      goto done;
    }
  }
