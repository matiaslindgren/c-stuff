...
bool sl_string_from_utf8(
    struct sl_context ctx[static 1],
    struct sl_span utf8_data[const static 1],
    struct sl_string out[static 1]
) {
  ...
  struct sl_span concatenated = {0};
  if (!sl_span_concat(ctx, utf8_data, &terminator, &concatenated)) {
    return false;
  }
  *out = (struct sl_string){
      .length    = sl_unicode_length(utf8_data),
      .utf8_data = concatenated,
  };
  return true;
}
...
void sl_string_copy_ascii(char dst[const static 1], const struct sl_string str[const static 1]) {
  if (str->utf8_data.size > 0) {
    memcpy(dst, str->utf8_data.data, str->utf8_data.size);
  }
}

bool sl_string_concat(
    struct sl_context ctx[static 1],
  ...
  if (!sl_span_concat(ctx, &str1_data, &str2_data, &data)) {
    return false;
  }
  if (!sl_string_from_utf8(ctx, &data, out)) {
...
bool sl_string_extend(
    struct sl_context ctx[static 1],
    struct sl_string str1[static 1],
    struct sl_string str2[const static 1]
) {
  struct sl_string result = {0};
  if (!sl_string_concat(ctx, str1, str2, &result)) {
    return false;
  }
...
bool sl_string_slice(
  ...
  return sl_string_from_utf8(ctx, &utf8_slice, out);
}
