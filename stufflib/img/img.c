...
void sl_img_segment_rgb(
      ...
      for (size_t col = 2; col < width - 1; ++col) {
        ...
        if (left_seg != cur_seg) {
          ...
          const double* left_diff
              = sl_math_linalg_vsub(bytes_per_px, (double[3]){0}, left_avg, cur_avg);
        ...
        if (above_seg != cur_seg) {
          ...
          const double* above_diff
              = sl_math_linalg_vsub(bytes_per_px, (double[3]){0}, above_avg, cur_avg);
  ...
  for (size_t row = 0; row < height; ++row) {
    for (size_t col = 0; col < width; ++col) {
      ...
      sl_png_image_set_pixel(dst, row, col, sl_png_image_get_pixel(src, src_row, src_col));
    }
  }
